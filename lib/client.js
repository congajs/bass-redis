...
const { QueryResult } = require('bass');
...
module.exports = class Client {
     ...
     *     my-database:my-collection-name:id
     *
    ...
    updateBy(metadata, collection, criteria, data, cb) {
        ...
        // @todo - convert all of this to a Lua script
        this.findBy(metadata, collection, criteria, null, null, null, (err, documents) => {
            ...
            documents.forEach((doc) => {
                ...
                const key = this.generateNewKey(collection, doc[metadata.getIdFieldName()]);

                calls.push((cb) => {
                    this.db.connection.hmset(key, doc, (err, res) => {
                        cb();
                    });
                });
            ...
            async.series(calls, () => {
                ...
                this.logger.debug(
                    '[bass-redis] - updateBy [' + collection + ']: ' + JSON.stringify(criteria)
    ...
    removeBy(metadata, collection, criteria, cb) {
        ...
        this.findBy(metadata, collection, criteria, null, null, null, (err, documents) => {
            ...
            documents.forEach((doc) => {
                ...
                keys.push(this.generateNewKey(
                    collection,
                    doc[metadata.getIdFieldName()]
                ));

            });

            this.db.connection.del(keys, (err, numRemoved) => {
                this.logger.debug(
                    '[bass-redis] - removeBy [' + collection + ']: ' +
    ...
    findCountByQuery(metadata, collection, query, cb) {
        ...
        this.findByQuery(metadata, collection, query, (err, result) => {
            cb(err, result.totalRows);
        });
    }
